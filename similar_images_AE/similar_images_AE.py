...
import numpy as np
sys.path.append("src")
from src.autoencoders.AE import AE
from src.clustering.KNN import KNearestNeighbours
from src.utilities.image_utilities import ImageUtils
from src.utilities.sorting import find_topk_unique
from src.utilities.plot_utilities import PlotUtils
...
def main():
    ...
    img_shape = (150, 150)  # force resize -> (ypixels, xpixels)
    ...
    n_neighbors = 20  # number of nearest neighbours
    ...
    query_dir = os.path.join(project_root, 'test2')
    #shutil.copy(path, query_dir)
    ...
    img_inventory_dir = os.path.join(db_dir, 'processed_inventory')
    if not os.path.exists(img_inventory_dir):
        os.makedirs(img_inventory_dir)
    ...
    if train_autoencoder:
        ...
        dictfn = MODEL.generate_naming_conventions(model_name+'final_dataset_test_arch2_0103', models_dir)
